...
func (b *ServiceBuilder) Build(cfg *model.Config, db *sql.DB) (*Container, error) {
	...
	// 将现有配置加载到服务中
	configService.Get()
	configService.Load() // 这会重新加载配置
	b.container.Register("config_service", configService)
	...
	// 创建并注册数据库服务
	// 使用新的ConfigService获取数据库配置
	dbConfig := configService.GetDatabaseConfig()
	poolConfig := database.PoolConfig{
		GuildsDBPath:     dbConfig.GuildsDBPath,
		UserDBPath:       dbConfig.UserDBPath,
		KickUserDBPath:   dbConfig.KickUserDBPath,
		TimedTasksDBPath: dbConfig.TimedTasksDBPath,
	}
	dbService := database.NewService(poolConfig)
	b.container.Register("database_service", dbService)

	// 创建并注册Repository管理器
	repositoryManager := repository.NewRepositoryManager(dbService)
	b.container.Register("repository_manager", repositoryManager)
